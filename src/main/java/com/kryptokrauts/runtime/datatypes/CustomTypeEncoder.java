...
public class CustomTypeEncoder extends AbstractDatatypeEncoder {
	...
	public CodeBlock encodeValue(TypeName type, String variableName) {
		return CodeBlock.builder()
				.add("$T." + CustomTypesGenerator.M_ENCODE_VALUE + "($L)", type,
						variableName)
				.build();
	}
	...
	public CodeBlock mapToReturnValue(TypeName type, String variableName) {
		return CodeBlock.builder().add(
				"$T." + CustomTypesGenerator.M_MAP_TO_RETURN_VALUE + "($L)",
				type, variableName).build();
	}
	...
	private String getCustomTypeName(Object type) {
		if (type != null && (type.toString().toLowerCase().startsWith(
				this.resolveInstance.generatedContractClassName.toLowerCase())
				|| CustomTypesGenerator.PREDEFINED_TYPES.keySet()
