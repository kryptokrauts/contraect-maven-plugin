...
public class AddressDatatypeGenerator {
  ...
  public void generate() throws MojoExecutionException {
    try {
      TypeSpec addressTypeSpec =
          TypeSpec.classBuilder(ADDRESS_DATATYPE_NAME)
      ...
      JavaFile javaFile = JavaFile.builder(config.getDatatypePackage(), addressTypeSpec).build();

      Path path = Paths.get("", config.getTargetPath());

      javaFile.writeTo(path);
    } catch (Exception e) {
      throw new MojoExecutionException("Error generating sophia address datatype", e);
    }
  }
  ...
  private MethodSpec constructor() {
    MethodSpec constructor =
        MethodSpec.constructorBuilder()
            ...
            .addParameter(
                ParameterSpec.builder(String.class, addressVariable, Modifier.FINAL).build())
