...
public class TupleMapper extends AbstractDatatypeMapper {
	...
	public boolean applies(TypeName type) {
		if (type instanceof ParameterizedTypeName) {
			return ((ParameterizedTypeName) type).rawType.packageName()
	...
	/**
	 * tuples are mapped as one string, separated by comma and enclosed by
	 * brackets: ("hello",0,true)
	 */
	public CodeBlock encodeValue(TypeName type, String variableName) {
		AtomicInteger index = new AtomicInteger(0);
		return CodeBlock.builder().add("\"(\"+").add(this.getInnerTypes(type)
				.stream()
		...
		if (type != null) {
			if (type instanceof JsonObject) {
				Object list = JsonObject.mapFrom(type)
						.getValue(JSON_TUPLE_IDENTIFIER);
				if (list instanceof JsonArray) {
					JsonArray tupleTypesArray = (JsonArray) list;
					return ParameterizedTypeName.get(
							ClassName.get(
									getTupleSizeClass(tupleTypesArray.size())),
	...
	private TypeName[] getTupleTypes(JsonArray innerTypes) {
		return innerTypes.stream()
				.map(innerType -> this.resolveInstance
						.getTypeNameFromJSON(innerType))
