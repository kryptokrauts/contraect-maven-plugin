...
public class ContraectGenerator {
	...
	private MethodSpec parseFunctionToMethodSpec(
		...
		if (!isVoid) {
			returnCodeBlock = mapResultCodeblock(VAR_RESULT_OBJECT, resultType,
					functionName);
		}
	...
	private CodeBlock mapResultCodeblock(String VAR_RESULT_OBJECT,
			TypeName resultType, String functionName) {
		...
		List<ParameterSpec> params = functionDescription
				...
				.stream().map(param -> {
					...
					return ParameterSpec.builder(this.datatypeEncodingHandler
							.getTypeNameFromJSON(paramMap.getValue(config
