...
public class ContraectGenerator {
	...
	@NonNull
	private ABIJsonConfiguration abiJsonConfiguration;
	...
	public void generate(String aesFile) throws MojoExecutionException {
		...
		ACIResult abiContent = codegenConfiguration
				.getAeternityService().compiler.blockingGenerateACI(aesContent,
						null, null);
		// ImmutableMap.of(
		// "Library.aes",
		// "namespace Library =\r\n"
		...
		if (abiContent.getEncodedAci() != null) {
			...
			throw new MojoExecutionException(CodegenUtil.getBaseErrorMessage(
					CmpErrorCode.FAIL_PARSE_ROOT,
					String.format(
							"Compiler failed to create ABI for contract %s",
							aesFile),
					Arrays.asList(
							Pair.with("AeAPIErrorMessage",
									abiContent.getAeAPIErrorMessage()),
							Pair.with("RootErrorMessage",
	...
	private void generateContractClass(JsonObject abiJson, String aesContent)
			throws MojoExecutionException {
		try {
			String className = CodegenUtil.getUppercaseClassName(abiJson
					.getString(abiJsonConfiguration.getContractNameElement()));

			this.datatypeEncodingHandler = new DatatypeMappingHandler(
					codegenConfiguration.getTargetPackage(), className);
			this.customTypesGenerator = new CustomTypesGenerator(
					this.datatypeEncodingHandler, this.abiJsonConfiguration);

			this.datatypeEncodingHandler
					.setCustomTypesGenerator(this.customTypesGenerator);
			...
			TypeSpec contractTypeSpec = TypeSpec.classBuilder(className)
					...
					.addField(AeternityServiceConfiguration.class, GCV_CONFIG,
							Modifier.PRIVATE)
					.addField(FieldSpec
							.builder(TypeName.BOOLEAN, GCV_CHECK_BYTECODE,
									Modifier.PRIVATE)
					...
					.addField(FieldSpec
							...
							.initializer("$T.getLogger($L)",
									LoggerFactory.class,
									abiJson.getString(abiJsonConfiguration
											.getContractNameElement())
											+ ".class")
							.build())
					.addField(FieldSpec
							.builder(int.class, GCV_NUM_TRIALS,
									Modifier.PRIVATE)
							.initializer("$L",
									codegenConfiguration.getNumTrials())
							.build())
					.addMethods(buildContractPrivateMethods())
					.addMethods(buildConstructor())
					.addMethods(this.buildContractMethods(abiJson))
					.addTypes(this.customTypesGenerator.generateCustomTypes(
							abiJson.getJsonArray(abiJsonConfiguration
									.getCustomTypeElement()),
							abiJson.getValue(
									abiJsonConfiguration.getStateElement())))
					.addJavadoc(LICENSE_HEADER).addJavadoc(KRYPTOKRAUTS)
					.addJavadoc(LICENSE).addJavadoc(GENERATED)
					.addJavadoc(SUPPORT).build();
			...
			JavaFile javaFile = JavaFile
					.builder(codegenConfiguration.getTargetPackage(),
							contractTypeSpec)
					.build();
			...
			throw new MojoExecutionException(CodegenUtil.getBaseErrorMessage(
					...
					String.format("Error generating contract %s",
							abiJson.getString(abiJsonConfiguration
									.getContractNameElement())),
	...
	private List<MethodSpec> buildConstructor() {
		...
		CodeBlock codeBlock = CodeBlock.builder()
				...
				.addStatement("this.$L = new $T().getService($L)",
						...
						PARAM_AETERNITY_SERVICE_CONFIGURATION)
				.addStatement("this.$L = $L", GCV_CHECK_BYTECODE,
						GCV_CHECK_BYTECODE)
		...
		return Arrays.asList(
				MethodSpec.constructorBuilder().addParameters(Arrays.asList(
						ParameterSpec
								.builder(AeternityServiceConfiguration.class,
										PARAM_AETERNITY_SERVICE_CONFIGURATION)
								.build(),
						ParameterSpec
								.builder(String.class, GCV_DEPLOYED_CONTRACT_ID)
								.build(),
						ParameterSpec
								.builder(TypeName.BOOLEAN, GCV_CHECK_BYTECODE)
								.build()))
	...
	private List<MethodSpec> buildContractMethods(JsonObject abi) {
		...
		if (abi != null && abi
				.getValue(abiJsonConfiguration.getFunctionsElement()) != null) {
			JsonArray functions = abi
					.getJsonArray(abiJsonConfiguration.getFunctionsElement());
	...
	private MethodSpec parseFunctionToMethodSpec(
		...
		String functionName = replaceInvalidChars(functionDescription
				.getString(abiJsonConfiguration.getFunctionNameElement()));

		if (codegenConfiguration.getInitFunctionName()
		...
		boolean isStateful = functionDescription
				.getBoolean(abiJsonConfiguration.getFunctionStatefulElement());
		boolean isPayable = false;
		ParameterSpec amount = null;

		if (functionDescription
				.containsKey(abiJsonConfiguration.getPayableElement())) {
			if (functionDescription
					.getBoolean(abiJsonConfiguration.getPayableElement())) {
				isPayable = true;
				amount = ParameterSpec
						.builder(TypeName.get(BigInteger.class), VAR_CC_AMOUNT)
						.build();
			}
		}

		// resolve the return type
		TypeName payloadType = this.datatypeEncodingHandler
				.getTypeNameFromJSON(functionDescription.getValue(
						abiJsonConfiguration.getFunctionReturnTypeElement()));
		...
		if (isStateful) {
			if (isVoid) {
				...
				returnType = ParameterizedTypeName.get(
						ClassName.get(Pair.class), TypeName.get(String.class),
						payloadType);
			}
		}

		CodeBlock returnCodeBlock = mapResultCodeblock(VAR_RESULT_OBJECT,
				VAR_CC_POST_TX_RESULT, payloadType, functionName, isStateful,
				isVoid);
		...
		CodeBlock codeBlock = CodeBlock.builder()
				...
				.addStatement("$T $L = this.$N($L,$S)",
						DryRunTransactionResult.class, VAR_DR_RESULT,
						GCPM_DRY_RUN, VAR_CC_MODEL, functionName)
				.addStatement(
		...
		if (isStateful) {
			codeBlock = codeBlock.toBuilder()
					...
					.add(returnCodeBlock).endControlFlow()
		...
		MethodSpec method = MethodSpec.methodBuilder(functionName)
				.addParameters(params).addCode(codeBlock)
				.addJavadoc(isStateful ? "Stateful function" : "")
				.returns(returnType).addJavadoc("")
	...
	private CodeBlock mapResultCodeblock(String VAR_RESULT_OBJECT,
			String VAR_POST_TX_RESULT, TypeName resultType, String functionName,
			boolean isStateful, boolean isVoid) {
		String VAR_UNWRAPPED_RESULT_OBJECT = "unwrappedResultObject";
		String VAR_RESULT_JSON_MAP = "resultJSONMap";

		CodeBlock mappedToReturnValue = this.datatypeEncodingHandler
				.mapToReturnValue(resultType, VAR_UNWRAPPED_RESULT_OBJECT);

		CodeBlock returnStatement = CodeBlock.builder()
				.addStatement("return $L", mappedToReturnValue).build();

		if (isStateful) {
			if (isVoid) {
				returnStatement = CodeBlock.builder().addStatement(
						"return $L.getTxHash()", VAR_POST_TX_RESULT).build();
			} else {
				returnStatement = CodeBlock.builder()
						.add("return $T.with($L.getTxHash(),", Pair.class,
								VAR_POST_TX_RESULT)
						.add(mappedToReturnValue).add(");").build();
			}
		}
		...
		return CodeBlock.builder()
				...
				.beginControlFlow("if($L instanceof $T)",
						VAR_UNWRAPPED_RESULT_OBJECT, Map.class)
				.addStatement("$T $L = $T.mapFrom($L)", JsonObject.class,
						VAR_RESULT_JSON_MAP, JsonObject.class,
						VAR_UNWRAPPED_RESULT_OBJECT)
				.beginControlFlow("if($L.containsKey($S))", VAR_RESULT_JSON_MAP,
						codegenConfiguration.getResultAbortKey())
				.addStatement("throw new $T($T.format($S,$S,$L.getValue($S)))",
						AException.class, String.class,
						"An error occured calling function %s: %s",
						functionName, VAR_RESULT_JSON_MAP,
						codegenConfiguration.getResultAbortKey())
				.endControlFlow().endControlFlow().add(returnStatement).build();
	}
	...
	private List<ParameterSpec> getParameterSpecFromSignature(
		...
		List<ParameterSpec> params = functionDescription
				.getJsonArray(
						abiJsonConfiguration.getFunctionArgumentsElement())
				.stream().map(param -> {
					JsonObject paramMap = JsonObject.mapFrom(param);
					return ParameterSpec.builder(
							this.datatypeEncodingHandler.getTypeNameFromJSON(
									paramMap.getValue(abiJsonConfiguration
											.getFunctionArgumentTypeElement())),
							replaceInvalidChars(
									paramMap.getString(abiJsonConfiguration
											.getFunctionArgumentNameElement())))
							.build();
	...
	private MethodSpec buildDeployMethod(JsonObject functionDescription) {
		...
		GCPM_DEPLOY = MethodSpec.methodBuilder("deploy")
				...
				.returns(ParameterizedTypeName.get(ClassName.get(Pair.class),
	...
	private List<MethodSpec> buildContractPrivateMethods() {
		return Arrays.asList(buildContractExistsMethod(),
				...
				buildWaitForTxMethod(), buildWaitForTxInfoMethod(),
		...
		this.GCPM_DEPLOY_CONTRACT = MethodSpec.methodBuilder("deployContract")
				...
				.addCode(CodeBlock.builder().addStatement(
						...
						.addStatement(
								...
								Pair.class, VAR_CC_POST_TX_RESULT,
								VAR_CC_POST_TX_INFO)
						.build())
				.returns(ParameterizedTypeName.get(ClassName.get(Pair.class),
						TypeName.get(String.class), TypeName.get(String.class)))
	...
	private MethodSpec buildWaitForTxInfoMethod() {
		...
		this.GCPM_WAIT_FOR_TX_INFO = MethodSpec.methodBuilder("waitForTxInfo")
				...
				.addCode(CodeBlock.builder()
						.addStatement("this.$N($L)", GCPM_WAIT_FOR_TX,
								MP_TXHASH)
						.addStatement(
	...
	private MethodSpec buildWaitForTxMethod() {
		...
		this.GCPM_WAIT_FOR_TX = MethodSpec.methodBuilder("waitForTx")
				.addParameter(
						ParameterSpec.builder(String.class, MP_TXHASH).build())
				.addCode(CodeBlock.builder()
						...
						.addStatement("$L.debug($S+$L)", GCV_LOGGER,
								"Transaction included in block: ", VAR_MINED_TX)
		...
		this.GCPM_CREATE_CCM = MethodSpec
				...
				.addParameters(Arrays.asList(
						...
						ParameterSpec.builder(ParameterizedTypeName
								.get(List.class, String.class), MP_PARAMS)
								.build(),
						ParameterSpec.builder(TypeName.get(BigInteger.class),
								MP_AMOUNT).build()))
				.addCode(CodeBlock.builder()
						...
						.addStatement("return $T.builder().callData($L)"
								...
								BaseConstants.class, MP_AMOUNT, MP_AMOUNT,
								BigInteger.class, GCPM_NEXT_NONCE, GCV_CONFIG,
								BigInteger.class, GCV_CONFIG)
						.build())
				...
				.addParameters(Arrays.asList(
						ParameterSpec
								.builder(ContractCallTransactionModel.class,
										MP_CC_MODEL)
								.build(),
						ParameterSpec.builder(String.class, MP_CC_FUNC_NAME)
								.build()))
				.addCode(CodeBlock.builder().addStatement(
						...
						.addStatement(
								...
								MP_CC_FUNC_NAME, VAR_DR_RESULTS, VAR_DR_RESULTS,
	...
	private String readFile(String filePath) throws MojoExecutionException {
		try {
			...
			throw new MojoExecutionException(CodegenUtil.getBaseErrorMessage(
					...
					String.format("Cannot read contract from file %s",
							filePath),
					Arrays.asList(Pair.with("Exception", e))));
		}
	}

	private String GENERATED = String.format(
	...
	private String SUPPORT = "If you like this project we would appreciate your support.\r\nYou can find multiple ways to support us here\r\nhttps://kryptokrauts.com/support";

	private String KRYPTOKRAUTS = " _                     _        _                    _                            \r\n"
			+ "| |                   | |      | |                  | |                           \r\n"
