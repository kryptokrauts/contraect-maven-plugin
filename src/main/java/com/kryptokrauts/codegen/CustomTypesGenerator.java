...
public class CustomTypesGenerator {
  ...
  public static final Map<String, CustomType> PREDEFINED_TYPES =
      ImmutableMap.of(CustomType.ADDRESS_TYPE, new AddressType(), CustomType.HASH_TYPE,
          new HashType(), CustomType.SIGNATURE_TYPE, new SignatureType(), CustomType.ORACLE_TYPE,
          new OracleType(), CustomType.ORACLE_QUERY_TYPE, new OracleQueryType());
  ...
  public List<TypeSpec> generateCustomTypes(JsonArray customTypes, Object state) {
    ...
    if (state != null) {
      customTypes.add(new JsonObject()
          .put(abiJsonConfiguration.getCustomTypeNameElement(),
  ...
  private TypeSpec generateCustomType(JsonObject typeDefinition) {
    ...
    if (fieldDefinition instanceof JsonObject) {
      ...
      if (record != null) {
        fields = record.stream().map(definition -> {
          ...
          String name = oneField.getString(abiJsonConfiguration.getCustomTypeTypedefNameElement());
          TypeName type = this.datatypeEncodingHandler.getTypeNameFromJSON(
              oneField.getValue(abiJsonConfiguration.getCustomTypeTypedefTypeElement()));
      ...
      throw new RuntimeException(CodegenUtil.getBaseErrorMessage(
          CmpErrorCode.UNFORSEEN_CUSTOM_TYPEDEF, "This is an unforseen custom type definition",
          Arrays.asList(Pair.with("TypeDefinition", typeDefinition.encodePrettily()))));
    }
    String name = CodegenUtil.getUppercaseClassName(
    ...
    return TypeSpec.classBuilder(name).addFields(buildFields(fields)).addFields(additionalFields)
        ...
        .addMethod(generateEncodeValueMethod(fields, name)).addMethod(generateToString(fields))
        .addMethod(generateEquals(fields, name))
        .addMethod(generateMapToReturnValueMethod(fields, name, isAlias)).build();
  }

  private MethodSpec generateEncodeValueMethod(List<Pair<String, TypeName>> fields,
      String customTypeName) {
    CodeBlock encodeValueLogic = CodeBlock.builder().add(" \"{")
        .add(fields.stream()
            .map(f -> f.getValue0() + "=\"+"
                + this.datatypeEncodingHandler.encodeParameter(f.getValue1(),
                    MP_PARAM + "." + f.getValue0())
                + "+\"")
            .collect(Collectors.joining(",")))
        .add("}\"").build();
    // if it is a predefined class use this encoding logic
    if (INSTANCE_PREDEFINED_TYPES.get(customTypeName.toLowerCase()) != null) {
      encodeValueLogic = INSTANCE_PREDEFINED_TYPES.get(customTypeName.toLowerCase())
          .encodeValueCodeblock(MP_PARAM);
    }

    return MethodSpec.methodBuilder(M_ENCODE_VALUE).addCode("return ").addCode(encodeValueLogic)
        .addCode(";").addModifiers(Modifier.PUBLIC, Modifier.STATIC)
        .addParameter(this.getParameterSpec(customTypeName)).returns(TypeName.get(String.class))
        .build();
  }

  private MethodSpec generateMapToReturnValueMethod(List<Pair<String, TypeName>> fields,
      String customTypeName, boolean isAlias) {
    CodeBlock returnValueLogic;
    if (isAlias) {
      returnValueLogic = CodeBlock.builder()
          .addStatement("return new $T($L)", this.getClassName(customTypeName),
              this.datatypeEncodingHandler.mapToReturnValue(fields.get(0).getValue1(), MP_PARAM))
          .build();
    } else {
      returnValueLogic = CodeBlock.builder().addStatement("return $T.mapFrom($L).mapTo($T.class)",
          JsonObject.class, MP_PARAM, this.getClassName(customTypeName)).build();
    }
    // if it is a predefined class use this encoding logic
    if (INSTANCE_PREDEFINED_TYPES.get(customTypeName.toLowerCase()) != null) {
      returnValueLogic =
          CodeBlock
              .builder().add("return ").add(INSTANCE_PREDEFINED_TYPES
                  .get(customTypeName.toLowerCase()).mapToReturnValueCodeblock(MP_PARAM))
              .add(";").build();
    }
    ...
    return MethodSpec.methodBuilder(M_MAP_TO_RETURN_VALUE)
        ...
        .returns(this.getClassName(customTypeName)).addCode(returnValueLogic).build();
  }
    ...
    MethodSpec valueConstructor =
        MethodSpec.constructorBuilder()
            .addParameters(fields.stream()
                ...
                .collect(Collectors.toList()))
            .addCode(generateConstructorInitializer(fields)).addModifiers(Modifier.PUBLIC).build();
  ...
  private CodeBlock generateConstructorInitializer(List<Pair<String, TypeName>> fields) {
    ...
    for (Pair<String, TypeName> pair : fields) {
      initializer = initializer.toBuilder()
          .addStatement("this.$L = $L", pair.getValue0(), pair.getValue0()).build();
    }
    return initializer;
  }
  ...
  private MethodSpec generateToString(List<Pair<String, TypeName>> fields) {
    return MethodSpec
        .methodBuilder("toString").returns(
            TypeName.get(String.class))
        .addModifiers(Modifier.PUBLIC)
        .addStatement("return " + ((fields != null && fields.size() > 0) ? fields.stream()
            .map(f -> "\"" + f.getValue0() + "=\"+this." + f.getValue0() + ".toString()")
            .collect(Collectors.joining("+\",\"+")) : "\"\""))
        .build();
  }
  ...
  private MethodSpec generateEquals(List<Pair<String, TypeName>> fields, String customTypeName) {
    ...
    return MethodSpec.methodBuilder("equals").returns(TypeName.BOOLEAN)
        .addModifiers(Modifier.PUBLIC).addParameter(TypeName.OBJECT, MP_OBJ)
        ...
        .addStatement(fields.stream().map(f -> {
          if (f.getValue1().isPrimitive()) {
            return CodeBlock.builder()
                .addStatement("return if($L != $L.$L )", f.getValue0(), MP_OTHER, f.getValue0())
                .build().toString();
          } else
            return CodeBlock.builder().beginControlFlow("if($L == null)", f.getValue0())
                .addStatement("if($L.$L == null) return false", MP_OTHER, f.getValue0())
                .endControlFlow().addStatement("else if(!$L.equals($L.$L)) return false",
                    f.getValue0(), MP_OTHER, f.getValue0())
                .build().toString();
        }).collect(Collectors.joining())).addStatement("return true").build();
  }

  private List<MethodSpec> generateCustomTypeGetters(List<Pair<String, TypeName>> fields) {
    return fields.stream()
        .map(field -> MethodSpec
            .methodBuilder("get" + field.getValue0().substring(0, 1).toUpperCase()
                + field.getValue0().substring(1))
            .addModifiers(Modifier.PUBLIC).returns(field.getValue1())
            .addStatement("return this.$L", field.getValue0()).build())
        .collect(Collectors.toList());
  }
  ...
  private List<MethodSpec> generateCustomTypeSetters(List<Pair<String, TypeName>> fields,
    ...
    return fields.stream()
        .map(field -> MethodSpec
            .methodBuilder("set" + field.getValue0().substring(0, 1).toUpperCase()
                + field.getValue0().substring(1))
            .addModifiers(Modifier.PUBLIC)
            .addStatement("this.$L = $L", field.getValue0(), field.getValue0())
            .addParameter(field.getValue1(), field.getValue0()).build())
        .collect(Collectors.toList());
  }

  private List<FieldSpec> buildFields(List<Pair<String, TypeName>> fields) {
    return fields.stream().map(
        field -> FieldSpec.builder(field.getValue1(), field.getValue0(), Modifier.PRIVATE).build())
        .collect(Collectors.toList());
  }
